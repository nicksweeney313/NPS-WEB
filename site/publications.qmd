...
(async function () {
  ...
  function matchesQuery(p, q) {
    if (!q) return true;
    const hay = [
      p.date, p.title, p.authors, p.journal, p.keywords, p.webnote, p.doi_url
    ].join(" ").toLowerCase();
  ...
  try {
    ...
    const thNotes    = makeSortableTh("Notes", "webnote");
    ...
    function renderBody() {
      ...
      for (const p of pubsView) {
        ...
        // Title (+ authors) — title becomes DOI link if present
        ...
        tdNotes.textContent = escapeText(p.webnote);
